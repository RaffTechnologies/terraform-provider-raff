...
> **18 resources, 44 data sources** covering the full Raff public API: compute (VMs, volumes, snapshots, backups, backup schedules), Kubernetes (clusters and node pools, with the version and node-plan catalogs), networking (VPCs, IPs, security groups), identity (projects, members, roles, API keys, SSH keys), and read-only catalogs (regions, templates, pricing, plus VPC CIDR suggestions, security-group templates, and the permission catalog). Built on [raff-go](https://github.com/RaffTechnologies/raff-go).

> **What's new in v0.3.2** — the provider no longer fights the cluster autoscaler. On a pool with `autoscale_enabled = true`, `node_count` is the starting size only: it is no longer refreshed into state and no longer pushed back on apply, so the autoscaler's decisions survive. `raff_k8s_cluster` follows the same rule for `default_pool` and stops correcting `version_id` while an automatic upgrade mode is set. Deleting a node pool now waits for the teardown to finish, so replacing a pool (a `plan_id` change) no longer fails on the old pool still holding the name. The User-Agent finally reports the real provider version, which was pinned at 0.1.1 in every previous release. Earlier 0.3.x: per-pool `plan_id`, and a release-build fix. Full details in the [API changelog](https://docs.rafftechnologies.com/api-reference/changelog).
